...
class AssetsTracker {
    ...
    async track(params) {
        for (const [collectionName, assetId] of Object.entries(params)) {
            ...
            utils_1.logger.debug(`Tracked asset ID ${assetId} in collection ${collectionName}`);
        }
    }
    /**
    ...
    async cleanup(collectionsToCleanup) {
        ...
        if (!collections.length) {
            utils_1.logger.warn('No collections specified for cleanup.');
            return;
        }
        for (const collectionName of collections) {
            const ids = this.trackedAssets[collectionName] || [];
            if (!ids.length) {
                utils_1.logger.warn(`No tracked assets found in collection: ${collectionName}`);
                continue;
            }
            try {
                const collection = this.db.collection(collectionName);
                const result = await collection.deleteMany({ _id: { $in: ids } });
                utils_1.logger.info(`Deleted ${result.deletedCount}/${ids.length} assets from collection ${collectionName}`);
            }
            catch (error) {
                utils_1.logger.error(`Error cleaning collection ${collectionName}:`, error);
            }
            this.trackedAssets[collectionName] = [];
        }
        utils_1.logger.info('Cleanup complete.');
    }
}
